...
/**
 * Created by chenwen on 2015/8/23.
 * 前后端共用的原型扩展/静态方法
 */
...
Date.prototype.format = function (fmt, day) {
    var o = {
...
/**
 * 从字串左侧移除指定字符，如果指定最小长度，则最终结果可能会保留一部分被指定为需要移除的字符
 * @param char
 * @param minLength
 * @returns {*}
 */
String.prototype.ltrim = function (char, minLength) {
    if (!char) {
        char = ' ';
    }
    ...
    // 模式匹配获得的字串
    var pattern = this.match(reg);

    if (!pattern) {
        return this;
    }
    var len = pattern[0].length;
    if (len + minLength > this.length) {
        reg = '';
        for (var i = 0; i < this.length - minLength; i++) {
            reg += char;
        }
        reg = new RegExp('^' + reg);
    }
...
/**
 * 添加元素至数组尾部，该元素将受到indexOf方法检查，如果检查结果>=0，则该方法将不作任何操作
 * @param el
 * @throw String 拒绝接受NaN
 */
Array.prototype.pushToSet = function (el) {
    if (typeof el === 'number' && isNaN(el)) {
        throw 'element can not be NaN';
    }
...
/**
 * 从数组中移除指定元素，如果不传入filter，则移除所有非真值
 * @param filter {?Object=} 如果为函数，则将数组元素逐个传递入该函数，如果函数返回值为真，则移除该元素;如果为数组，
 * 则根据该数组是否包含元素作为过滤依据
 * @returns {Array} 返回原数组经过处理后的拷贝
 */
Array.prototype.compact = function (filter) {
    if (!arguments.length) {
        return _.compact(this);
    }
    var res = [];
    this.forEach(function (elem) {
        if ((_.isFunction(filter) && filter(other)) || (_.isArray(filter) && filter.indexOf(elem) >= 0) ||
            filter === elem) {
            return;
        }
        res.push(elem);
